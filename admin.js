...
async function setFullname(course, name) {
  try {
...
async function toggleExtension(course) {
  try {
    const c = await models.Course.findOne({ name: course })
    console.log(c)
    c.extension = true
...
async function setWeekExercises(name, week, cnt) {
  try {
    const c = await models.Course.findOne({ name })
    const e = c.exercises.slice()
    e[week] = cnt
    c.exercises = e
    const x = await c.save()
    models.mongoose.connection.close()
  } catch (e) {
    console.log(e)
    models.mongoose.connection.close()
