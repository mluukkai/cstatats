Feature:  A new user account can be created if a proper unused username and a proper password are given
    ...
    Scenario: creation fails with correct username and too short password
        Given command new user is selected
        When  a valid username "liisa" and too short password "aa" are entered
        Then user is not created and error "password should have at least 8 characters" is reported   

    Scenario: creation fails when password and password confirmation do not match
        Given command new user is selected
        When  a valid username "liisa" and password "salainen1" and a non password confirmation  "salainen2" are entered
