...
const solutionFolder = (courseName, part) =>
  path.resolve(__dirname, `../assets/solutions/${courseName}/part${part}`)
    ...
    try {
      ...
      fs.readdirSync(folder).forEach((name) => {
        ...
        const fileObject = {
          name,
          type,
          fullName: fullName.split(courseSolutions)[1],
        }
        if (isDir(fullName)) {
...
const getSolutionFile = async (req, res) => {
  ...
  const completedParts = withExtendedSubmissions.submissions
    .filter((s) => s.courseName === courseName)
    .map((s) => Number(s.week))
  if (!completedParts.includes(Number(part)))
